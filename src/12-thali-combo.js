...
export function createThaliDescription(thali) {
  ...
  return  `${thaliname.toUpperCase()} (${isveg}) - Items: ${thaliitems.join(", ")} - Rs.${thaliprice.toFixed(2)}`
}
...
export function getThaliStats(thalis) {
  ...
  const priceArray = thalis.map((thali) => thali.price);
  const names = thalis.map((thali) => thali.name);

  const cheapest = Math.min(...priceArray);
  const costliest = Math.max(...priceArray);
