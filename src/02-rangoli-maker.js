...
export function repeatPattern(pattern, times) {
  if( typeof(pattern)!="string" || (times < 0 || typeof(times)!= 'number' || times%1 !=0)) return "";
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  let aftersep=colorString.split(oldSep);
  let afterjoin = aftersep.join(newSep);
  return afterjoin;
...
export function makeRangoliBorder(char, length) {
  ...
  let newchar = char.repeat(length);
